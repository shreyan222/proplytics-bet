...
from matchupscores import def_rank
import unicodedata
...
class EnhancedPropsProcessor:
    ...
    def fetch_all_team_positions(self, teams):
        ...
        team_depth_charts = {}
        position_cache = {}  # canonical_name -> position
            ...
            try:
                ...
                for col in range(len(depth_chart[0])):
                    for row in range(len(depth_chart)):
                        ...
                        key = self.canonical_name(player_name)

                        if key and key not in position_cache:
                            position_cache[key] = position_arr[row]
    ...
    def auto_upload_and_filter_props(self, props, league_name="", game_mapping=None):
        ...
        auto_upload_stat_types = [
            ...
            "Qtrs with 5+ Rec Yards",   
            "Halves with 100+ Pass Yds",
            "Halves with 25+ Rec Yds",
            "Halves with 25+ Rush Yds",
            "Passing Yards in First 10 Attempts",
            "Qtrs with 10+ Rec Yds",
            "Qtrs with 20+ Rush Yds",
            "Qtrs with 50+ Pass Yds",
    ...
    def apply_matchup_rank_to_score(self, props, league_name=""):
        """Apply matchup ranking adjustments to prop scores (NFL only)
        
        This should be called AFTER assign_matchup_ranks_to_props() has been called.
        Matchup rank is the MOST IMPORTANT factor for NFL props.
        
        Args:
            props: List of Prop objects with matchup_rank already assigned
            league_name: "NFL" or "NBA" (only NFL supported currently)
        """
        print(f"🎯 Applying matchup rank adjustments to {len(props)} {league_name} prop scores...")
        
        if league_name != "NFL":
            print(f"⚠️ Matchup rank scoring only applies to NFL props")
            return
        ...
        for prop in props:
            try:
                # Get matchup rank (should already be assigned)
                matchup_rank = getattr(prop, 'matchup_rank', 16)
                original_score = getattr(prop, 'score', 0)
                
                # Validate matchup rank
                if matchup_rank == 'N/A' or matchup_rank is None:
                    matchup_rank = 16  # Default to middle
                
                # Convert to int if needed
                try:
                    matchup_rank = int(matchup_rank)
                except (ValueError, TypeError):
                    matchup_rank = 16
                
                # Calculate matchup multiplier (1-32 scale where 32 is best matchup, 1 is worst)
                # Rank 32 (worst defense): 1.50x multiplier (50% boost)
                # Rank 16 (average defense): 1.00x multiplier (no change)
                # Rank 1 (best defense): 0.50x multiplier (50% penalty)
                
                # Linear interpolation: multiplier = 0.5 + (rank - 1) * (1.0 / 31)
                # This creates a range from 0.5 (rank 1) to 1.5 (rank 32) with 1.0 at rank 16.5
                matchup_multiplier = 0.5 + ((matchup_rank - 1) / 31.0) * 1.0
                
                # Apply the matchup multiplier to the score
                # Since matchup rank is MOST IMPORTANT, we give it significant weight
                new_score = original_score * matchup_multiplier
                
                # Update the prop score
                prop.score = round(new_score, 3)
                
                updated_count += 1
                
                # Log the update for verification
                print(f"📊 {prop.player_name} ({prop.stat_type} {prop.line_score}): "
                      f"Rank {matchup_rank}/32 → Multiplier {matchup_multiplier:.3f} → "
                      f"Score {original_score:.3f} → {prop.score:.3f}")
                
            except Exception as e:
                print(f"❌ Error applying matchup rank to {getattr(prop, 'player_name', 'Unknown')}: {e}")
                skipped_count += 1
                continue
        
        print(f"✅ Updated {updated_count} prop scores with matchup rank adjustments")
        if skipped_count > 0:
            print(f"⚠️ Skipped {skipped_count} props due to errors")
        ...
        try:
            ...
            if not nba_norm1.empty:
                ...
                if not nba_norm.empty:
                    nba_norm['Display Name'] = nba_norm['Display Name'].apply(self.normalize_text)
                    ...
                    if new_nba_props:
                        ...
                        if props_to_analyze:
                            ...
                            self.nba_processing(0, "since-2025-2026-season-during-regular-season", props_to_analyze, nba_norm, nba_game_mapping)
            ...
            if not nfl_norm.empty:
                ...
                if new_nfl_props:
                    ...
                    if props_to_analyze:
                        ...
                        # Apply matchup rank adjustments to scores (MOST IMPORTANT FACTOR)
                        self.apply_matchup_rank_to_score(props_to_analyze, "NFL")
    ...
    def nba_processing(self, num, timeframe, props, norm, game_mapping=None):
        ...
        headers = [
            ...
            'H2HArray', 'L5Array', 'l7_mr', 'l15_mr', 'l30_mr', 'FinalMatchupScore',
            'Temp', 'Size', 'H2HAvg', 'L5Avg', 'Diff',
        ...
        def map_stat(stat_raw):
            ...
            return lookup.get(stat_raw, "NA")

        def get_rank(pos, tf_alias, stat_key, team_key):
            return def_rank.get(pos.lower(), {}).get(tf_alias, {}).get(stat_key, {}).get(team_key, "NA")
        
        def compute_final_matchup_score(l7_rank, l15_rank, l30_rank):
            """Weighted matchup score: (3*l7 + 2*l15 + 1*l30)/6 * 3.33"""
            def to_num(v):
                return v if isinstance(v, (int, float)) else 16
            l7 = to_num(l7_rank)
            l15 = to_num(l15_rank)
            l30 = to_num(l30_rank)
        ...
        for i, prop in enumerate(props_sorted):
            try:
                ...
                stat_key = map_stat(prop.stat_type)
                defagainstteam = against_team
                if against_team == "PHX":
                    defagainstteam = "PHO"
                elif against_team == "UTA":
                    defagainstteam = "UTH"
                elif against_team == "NOP":
                    defagainstteam = "NOR"
                l7_rank = get_rank(position, "l7", stat_key, defagainstteam)
                l15_rank = get_rank(position, "l15", stat_key, defagainstteam)
                l30_rank = get_rank(position, "l30", stat_key, defagainstteam)
                final_matchup_score = compute_final_matchup_score(l7_rank, l15_rank, l30_rank)
                prop.matchup_ranks = {"l7": l7_rank, "l15": l15_rank, "l30": l30_rank}
                prop.matchup_rank = l7_rank if isinstance(l7_rank, (int, float)) else None
                prop.final_matchup_score = final_matchup_score
                ...
                try:
                    ...
                    L5relative_diff = round((L5avg - prop.line_score) / (prop.line_score), 3)
                    L5relative_diff = max(-0.25, min(0.25, L5relative_diff))
                    ...
                    skipped_props += 1
                    continue
                #print("🚧 PASSED L5 — entering H2H")
                try:
                    # Fetch H2H data (only when player changes)
                    if i <= 10 or current_player != previous_player:
                        #print(f"🔄 Fetching H2H data: {current_player} vs {against_team} ({timeframe})")
                        
                        try:
                            h2h_against_team = against_team
                            if against_team=="CHA":
                                h2h_against_team = "hornets"
                            h2harr = self.fetch_stats_instant(
                                lambda: dataFinder.stats_against_team_t_season(current_player, h2h_against_team, timeframe),
                    ...
                    if i <= 10 or prop.stat_type != previous_stat:
                ...
                if timeframe == "since-2024-2025-season-during-regular-season":
                    prop.add_performance_data(h2hstatarr, "H2H1Y")
                elif timeframe == "since-2025-2026-season-during-regular-season":
                    prop.add_performance_data(h2hstatarr, "H2H1Y")
                #L5temp = 0
                h2htemp = 0
                #h2hinjury = 0
                ...
                h2hrelative_diff = round((h2havg - prop.line_score) / (prop.line_score), 3)
                h2hrelative_diff = max(-0.25, min(0.25, h2hrelative_diff))
                h2hpercent = round(100 * (sum(h2hstatarr) / len(h2hstatarr) - prop.line_score) / prop.line_score, 3)
                sample_size = h2hsize
                h2h_score = (((h2htemp / h2hsize)*7 + h2hrelative_diff*1.33*3)/10)*(sample_size/(sample_size+6))*2.25
                l5_score = ((L5temp / 5)*7 + L5relative_diff*1.33*3)/9
                
                # Calculate sorting score (always calculate, regardless of H2H threshold)
                sorting_score = (
                        (h2h_score) * 4.5 +
                        (l5_score * 1.5) +
                        (final_matchup_score/25)
                )
                sorting_score = round(sorting_score, 3)
                prop.final_matchup_score = final_matchup_score
                # Assign the sorting score to the prop object
                prop.score = sorting_score
